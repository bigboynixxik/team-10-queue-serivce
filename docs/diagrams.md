...
Условное обозначение участников везде одинаковое: `A`, `B`, `C` — покупатели (браузер), `QS` — Queue Service (наш backend), `OS` — Order Service (внешний, вне скоупа кейса).
...
sequenceDiagram
    ...
    participant OS as Order Service (внешний, вне скоупа)
    ...
    A->>OS: create order (token передан, вне скоупа)
    Note over OS: Order Service сам проверяет право перед созданием заказа —<br/>отдельный запрос к Queue Service не нужен
    OS-->>A: форма оплаты
    A->>OS: оплата
    OS-->>QS: POST /rights/{token}/events { event: payment_succeeded, order_id }

    QS->>QS: right(A) = USED, product_count--
    QS-->>OS: 202 Accepted
...
sequenceDiagram
    ...
    opt Демонстрация: попытка обхода очереди
        B->>OS: create order (передаёт token_A — пробует обойти очередь чужим токеном)
        OS-->>B: 403 Forbidden
        Note over OS: Order Service проверяет владельца токена (внутренний вызов<br/>к Queue Service, вне скоупа) и отклоняет запрос
    end
    ...
    B->>OS: create order (token передан, вне скоупа)
    OS-->>B: форма оплаты
    B->>OS: оплата
    OS-->>QS: POST /rights/{token}/events { event: payment_succeeded, order_id }

    QS->>QS: right(B) = USED, product_count--
    QS-->>OS: 202 Accepted
...
sequenceDiagram
    ...
    alt A успевает оплатить до истечения таймера (этот сценарий)
        A->>OS: create order (token передан, вне скоупа)
        OS-->>A: форма оплаты
        A->>OS: оплата (успевает до истечения таймера)
        OS-->>QS: POST /rights/{token}/events { event: payment_succeeded, order_id }
...
sequenceDiagram
    ...
    Note over A: available_units = 3 после выдачи права A
    ...
    alt B соглашается на меньшее количество (этот сценарий)
        ...
        QS->>QS: right(B) = USED, product_count -= 2
        QS-->>OS: 202 Accepted
        QS-->>B: WS push { status: PURCHASED }
