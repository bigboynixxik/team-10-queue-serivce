...
Проверка права перед созданием заказа образует границу доверия механики очереди: без неё покупатель мог бы обратиться к оформлению заказа напрямую, минуя очередь (`docs/design_context.md`, п. 5.9).
